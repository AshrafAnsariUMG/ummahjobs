...
function StarRating({ rating }: { rating: number }) {
  return (
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map((star) => (
        <svg key={star} className="w-4 h-4" fill={star <= Math.round(rating) ? '#f59e0b' : '#e5e7eb'} viewBox="0 0 20 20">
...
export default function EmployerReviewsSection({ slug, initialReviews }: Props) {
  ...
  // Fetch fresh reviews on mount to bypass SSR cache
  useEffect(() => {
    fetch(`${API}/api/employers/${slug}/reviews`, { headers: { Accept: 'application/json' } })
      .then((r) => r.json())
      .then((data) => {
        const list: EmployerReview[] = Array.isArray(data) ? data : (data.reviews ?? [])
        setReviews(list)
      })
      .catch(() => {})
  }, [slug])
  ...
  return (
    <section>
      ...
      <div className="mb-6">
        {!user ? (
          <div style={{ background: '#F9FAFB', border: '1px solid #E5E7EB', borderRadius: '8px', padding: '20px', textAlign: 'center' }}>
            <p style={{ color: '#6B7280', margin: '0 0 12px' }}>
              Share your experience working with this employer
            </p>
            <a href="/login" style={{ padding: '8px 20px', background: '#033BB0', color: 'white', borderRadius: '6px', textDecoration: 'none', fontSize: '14px', fontWeight: '600', display: 'inline-block' }}>
          ...
          <div style={{
            ...
            borderRadius: '8px', padding: '16px 20px', fontSize: '14px',
            color: alreadyReviewed ? '#92400E' : '#166534',
          }}>
            {alreadyReviewed ? 'You have already reviewed this employer.' : 'You have reviewed this employer. JazakAllah Khayran!'}
          </div>
        ) : (
          ...
          {reviews.map((review) => (
            <div key={review.id} className="bg-white rounded-xl border border-gray-200 p-5">
              <div className="flex items-start justify-between gap-3 mb-2">
                <div>
                  <p className="text-sm font-semibold text-gray-900">{review.reviewer?.display_name ?? 'Anonymous'}</p>
