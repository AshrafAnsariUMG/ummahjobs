...
class JobController extends Controller
{
    ...
    public function update(Request $request, string $id)
    {
        ...
        // Quick-toggle mode (feature/expire from jobs list) — only status/is_featured/is_urgent sent
        $isQuickToggle = count($request->except(['_method', '_token'])) <= 3
            && !$request->has('title')
            && !$request->has('employer_type');
        ...
        if ($isQuickToggle) {
            ...
            RevalidationService::trigger();
            return response()->json($job->load(['employer:id,company_name,slug,logo_path', 'category:id,name']));
        }
        ...
        return response()->json($job->fresh(['employer:id,company_name,slug,logo_path', 'category:id,name']));
    }
